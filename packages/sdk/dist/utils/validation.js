 ...
 */
const NIMIQ_ADDRESS_REGEX = new RegExp(`^NQ\\d{2}[${NIMIQ_ALPHABET}]{32}(?:\\+[A-Za-z0-9-]{1,32})?$`);
 ...
 */
export function assertValidHandle(handle, label = 'handle') {
    const normalized = normalizeHandle(handle);
    if (normalized.length < HANDLE_MIN_LENGTH || normalized.length > HANDLE_MAX_LENGTH) {
        throw new NeetSDKError(`${label} must be between ${HANDLE_MIN_LENGTH} and ${HANDLE_MAX_LENGTH} characters`, { code: 'VALIDATION' });
    }
    if (!HANDLE_REGEX.test(normalized)) {
        throw new NeetSDKError(`${label} can only contain lowercase letters, numbers, underscores, and hyphens`, { code: 'VALIDATION' });
    }
    return normalized;
}
/**
 ...
 */
export function toFullHandle(handle) {
...
export function assertValidNimiqAddress(address, label = 'Nimiq address') {
